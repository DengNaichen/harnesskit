...
`mykit` 安装到目标仓库中的资产可以理解为面向 agent 的技术治理层，主要包括：
...
用物理类比说，经典力学必须先有坐标系；agent workflow 也必须先有仓库坐标系。`mykit` 当前先建立坐标系并安装约束，下一阶段才是在这个系统里求最速降线。换句话说：Scan 负责测绘地形，Harness 负责安装约束，Workflow 负责画出路径。

当前 `mykit` 的边界是 Scan + Harness，而不是 Workflow Builder。它可以帮助 agent 保持在正确的项目事实、规则和验证约束中，但不假装知道某个具体任务的最优轨道长什么样。这也是 MVP 的重要限制：`mykit` 维护的是执行任意 workflow 所需的稳定上下文和约束；任务级目标拆解、路径优化和 workflow 生成属于后续产品层。

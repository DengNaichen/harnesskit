# Harness Builder MVP 框架：Scan → Skill / Rule → Validation 循环
...
- [AGENTS.md](../../AGENTS.md)：agent 的操作规则和仓库导航入口
- 架构或质量文档：更深入的工程约束
- 生成或人工整理的参考资料：避免 agent 凭空猜测关键事实
- 验证入口：把自然语言规则连接到可执行检查

[AGENTS.md](../../AGENTS.md) 应该保持克制，专注于“agent 在这个仓库里应该怎么工作”。至于 HarnessKit 是什么、为什么存在、MVP 边界在哪里，这些项目定位说明应该放在 [README.md](../../README.md)。
...
MVP 暂不追求让 harness 在使用中自动进化成更完整的架构体系；这属于后续的 Harness Evolution。当前更重要的是 Harness Preservation：确认已有的 [AGENTS.md](../../AGENTS.md)、skills、配置、验证入口和文档引用仍然存在、可读、可解释，并且没有明显漂移。

---

## 3. 核心循环：Scan → Rule → Validation

### Skill、Rule 与 Validation 的分工

Context Harness 里的 agent-facing 资产应按职责拆开：

- **Skill** 教 agent 怎么做一类任务。它有触发条件、步骤、输入输出和验证方式，适合承载扫描、填充、兼容性判断、验证、PR 总结等流程。
- **Rule** 告诉 agent 在这个仓库里永远或局部必须遵守什么。它是约束，不是流程；应短、明确、可引用。
- **Validation** 负责把可检查的 Rule 接到 runner 上。agent 没遵守时，Validation 应尽量通过命令、hook、CI、平台 check 或 review 给出明确反馈；是否阻断取决于它绑定到哪个 runner。

[AGENTS.md](../../AGENTS.md) 是路由入口：它决定这轮任务要读哪些 context、触发哪些 skills、遵守哪些 rules、运行哪些 validations。[RULES.md](../../RULES.md) 不应该替代 skill，也不应该成为第二份 [AGENTS.md](../../AGENTS.md)；它更像可被 agent、skill、linter 和 Validation 共同引用的约束索引。
...
Scan → 读 git repo，提取项目事实（技术栈、构建工具、linter、CI 配置等）
  │
Skill → 按任务类型指导 agent 如何扫描、修改、验证和沉淀 context
  │
Rule → 基于事实沉淀项目级约束，写进 repo，引导 agent 不越界
  │
Validation → 确定性验证，通过 runner 执行并返回 pass/fail
  │
...
Agent 不需要一次做对，只需要收敛。Rule 让它大概往对的方向走，Validation 在每次 commit 时给出确定性的 pass/fail 反馈，agent 根据反馈修正，再试，直到通过。

---

## 4. Skill、Rule 与 Validation 的本质区分

| | Skill | Rule | Validation |
|---|---|---|---|
| **性质** | 流程性、任务型、可按需加载 | 约束性、仓库局部、概率性 | 确定性、可执行、可判定 |
| **作用** | 教 agent 如何完成一类任务 | 引导 agent 不破坏仓库约束 | 给出可执行反馈并交给 runner 决定是否阻断 |
| **验证方式** | 按步骤产出结果并运行对应检查 | 主要靠 agent 理解和遵循 | 可以跑一个命令，返回 pass/fail |
| **失败模式** | agent 没触发或没按流程做 | agent 可能忽略，但不一定能被机器发现 | 违反就一定失败或告警 |

**Skill 是任务流程。**

**Rule 是在概率空间里给模型施加的势场，影响模型的输出分布，但不能硬保证结果。**

**Validation 是确定性的守恒量本身，可验证、不依赖模型。**

核心设计原则：
- 流程放进 Skill，约束放进 Rule，机器检查放进 Validation。
- [AGENTS.md](../../AGENTS.md) 只做路由和完成门槛，不吞掉全部 Skill 或 Rule 内容。
- 每条 Validation 背后应有一条 Rule 解释"为什么有这个约束"。
- 每条 Rule 尽可能衍生出一个 Validation 来兜底。
- 能完全确定性验证的部分应下沉为 Validation；无法完全验证的部分才留在 Rule 或 review 层。

### Rule 的内容范围

Rule 的内容是**仓库中必须长期成立、但不一定能靠一条命令完整判定**的约束：
...
Validation 的内容是**可以用一条命令或一段确定性逻辑判定 pass/fail** 的东西：
...
上述工具串进 git pre-commit hook 或 CI 即为 doc lint Validation。
...
| **Validation** | ✅ 全自动（Scan 结果 → hook 生成） | ❌ | ❌ |
| **Doc Lint** | ✅ 全自动（组合现有工具） | ❌ | ❌ |
...
### Validation 从 Scan 结果自动推导

- 扫到 eslint → Validation 加 `npm run lint`
- 扫到 TypeScript → Validation 加 `npm run typecheck`
- 扫到 test 框架 → Validation 加 `npm test`
...
- 动态渲染不应替代 Git hook / pre-commit 这类确定性 Validation。
...
| 风险 | 评估 | 应对 |
|------|------|------|
| 模型进化可能压缩 Rule 层价值 | ⚠️ Rule 会被压缩，Validation 不会 | 产品重心偏 Validation |
...
- Validation 的执行锚点是否需要从 git hook 扩展为更抽象的验证入口（适配云端 agent 环境）

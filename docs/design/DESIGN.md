...
`AGENTS.md` 是路由入口：它决定这轮任务要读哪些 context、触发哪些 skills、遵守哪些 rules、运行哪些 validations。`RULES.md` 不应该替代 skill，也不应该成为第二份 `AGENTS.md`；它更像可被 agent、skill、linter 和 Validation 共同引用的约束索引。
...
- `AGENTS.md` 只做路由和完成门槛，不吞掉全部 Skill 或 Rule 内容。

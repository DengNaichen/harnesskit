...
Validation 是把 Rule 变成可验证约束的机制。`RULES.md` 说明团队和 agent 应该遵守什么；Validation 说明其中哪些部分可以被检查、如何检查、在哪里运行，以及 runner 具备什么执行或阻断强度。

本文件只说明 Validation 应该如何设计，不记录当前仓库已经绑定了哪些 Validation。具体项目的 Validation 状态、命令和 runner 证据应写在该项目自己的 `RULES.md`、agent 指南、验证入口或平台配置中。
...
- `RULES.md`：描述 Rule、适用范围、Validation 类型、Runner 证据和当前状态。

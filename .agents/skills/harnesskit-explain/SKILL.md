...
description: "解释当前仓库中的 HarnessKit Context Harness 资产和使用方式。"
---

# harnesskit-explain

当用户询问 HarnessKit 是什么、当前仓库为什么有这些 harness 文件，或这些文件分别给谁使用时，使用这个 skill。

## 工作方式

1. 优先读取 `README.md` 理解项目定位。
2. 读取 `.harnesskit/config.json` 判断当前安装状态。
3. 读取 `AGENTS.md` 理解 agent 操作规则。
4. 查看 `.agents/skills/` 说明当前 Codex 可用的 HarnessKit skills。

## 解释重点

<!-- harnesskit:todo-checklist:start -->
调整本节前请确认：
- 文件职责说明与当前模板输出和 `.harnesskit/config.json` 内容一致。
- 不把 `AGENTS.md` 当作产品说明来源，项目定位优先来自 `README.md`。
- 新增或移除 harness 资产后同步更新这里。
<!-- harnesskit:todo-checklist:end -->

- HarnessKit 是 Context Harness CLI 与 Codex-facing toolkit。
...
- `.harnesskit/config.json` 记录 HarnessKit 管理的最小元数据。

...
- [`src/harnesskit/`](src/harnesskit/) <!-- harnesskit:coverage=direct-children -->：打包后的 HarnessKit 运行时代码。修改 CLI、初始化行为、integration 安装、模板渲染或 `.harnesskit/config.json` 写入时看这里。
  ...
  - [`cli.py`](src/harnesskit/cli.py)：Typer 命令面，定义 `harnesskit init`、`harnesskit lint`、`harnesskit integration list`、`harnesskit integration install`、`--force`、`--here`、`--integration` 和 `--version`。
  - [`init.py`](src/harnesskit/init.py)：初始化引擎，负责解析项目路径、复制内置模板、用 `StrictUndefined` 渲染 Jinja 模板、写入 agent companion 文件、安装 Codex skills、校验 integration，并写入 `.harnesskit/config.json`。
  - [`linter/`](src/harnesskit/linter/) <!-- harnesskit:coverage=direct-children -->：打包进 CLI 的 Context Harness linter runtime，支撑 `harnesskit lint` 和 pre-commit harness 检查。
    - [`__init__.py`](src/harnesskit/linter/__init__.py)：标记 linter package。
    - [`core/`](src/harnesskit/linter/core/) <!-- harnesskit:coverage=direct-children -->：linter 规则共享的模型、常量、issue helper 和 Markdown 解析 helper。
      - [`__init__.py`](src/harnesskit/linter/core/__init__.py)：标记 core helper package。
      - [`constants.py`](src/harnesskit/linter/core/constants.py)：共享 linter 常量、支持的 integration、必需 Codex skill 路径、marker 字符串、命令字符串和正则模式。
      - [`issues.py`](src/harnesskit/linter/core/issues.py)：构造标准化 lint issue 的 helper，并把路径规范化为仓库相对路径。
      - [`markdown.py`](src/harnesskit/linter/core/markdown.py)：Markdown marker 和链接解析 helper，供多个规则模块复用。
      - [`models.py`](src/harnesskit/linter/core/models.py)：lint issue、report、severity 和 marked Markdown block 的 dataclass 模型。
    - [`rules/`](src/harnesskit/linter/rules/) <!-- harnesskit:coverage=direct-children -->：相互独立的 lint 规则组。
      - [`__init__.py`](src/harnesskit/linter/rules/__init__.py)：标记 rule package。
      - [`architecture.py`](src/harnesskit/linter/rules/architecture.py)：检查 `ARCHITECTURE.md` 的未完成职责描述、coverage hint、链接路径是否存在，以及 direct-child coverage。
      - [`config.py`](src/harnesskit/linter/rules/config.py)：检查 `.harnesskit/config.json` 的结构、schema version、默认 integration 和 installed integration 列表。
      - [`core.py`](src/harnesskit/linter/rules/core.py)：检查必需 harness 文件、`CLAUDE.md` 指针行为和已安装的 Codex integration 资产。
      - [`harness_markdown.py`](src/harnesskit/linter/rules/harness_markdown.py)：收集 harness Markdown 文件，并检查本地链接、TODO checklist marker 配对和可选外部 Markdown lint。
      - [`project.py`](src/harnesskit/linter/rules/project.py)：检查请求 lint 的目标是否为目录。
      - [`rule_details.py`](src/harnesskit/linter/rules/rule_details.py)：检查 `RULES.md` 中的短规则是否有对应 `.harnesskit/rules/RULE-*.md` details 文件，并暴露 orphan/过长规则摘要。
      - [`skills.py`](src/harnesskit/linter/rules/skills.py)：检查 skill frontmatter，以及 `AGENTS.md` 中的 `$skill-name` 引用是否存在对应 skill。
      - [`tech_stack.py`](src/harnesskit/linter/rules/tech_stack.py)：从 manifest 检测仓库技术栈事实，并检查已声明的 tech-stack block 是否漂移。
      - [`verification.py`](src/harnesskit/linter/rules/verification.py)：检查验证文档中的过期测试框架引用，以及 lint、format、build、pre-commit 命令是否被正确记录。
    - [`runner.py`](src/harnesskit/linter/runner.py)：linter 编排和报告输出层，运行所有 lint 规则组并输出文本或 JSON 报告。
...
- [`harness-linter-poc/`](harness-linter-poc/) <!-- harnesskit:coverage=direct-children -->：旧 Context Harness linter POC/参考实现。新增产品行为应优先落在 [`src/harnesskit/linter/`](src/harnesskit/linter/)。
...
- `CLAUDE.md`：指向 `AGENTS.md` 的 Claude Code companion 文件；`codex` 和 `claude` integration 当前都把它生成为符号链接。

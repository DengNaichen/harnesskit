...
当前 MVP 只聚焦 **Context Harness**；当前支持 `codex` 和 `claude` integration，其中 `codex` 是默认值。HarnessKit 不实现 agent runtime、工具沙箱、编排控制器，也不做长期运行 agent 的调度系统。

## 产品体验

HarnessKit 本体仍然是 CLI，但真正的使用体验不是让用户反复手敲脚本，而是把 CLI 和 agent skills 配合起来：

1. 用户通过 PyPI 分发包 `infharness` 安装 CLI。
2. 在某个仓库里运行 `harnesskit init --here`，或运行 `harnesskit init <project>`。
3. HarnessKit 初始化 Context Harness 核心资产。
4. 如果选择 `codex` integration，HarnessKit 同时安装 Codex skills 到 `.agents/skills/`；如果选择 `claude` integration，则安装 Claude Code companion 指南。
5. 用户回到对应 agent 对话框里，通过本地上下文继续扫描仓库事实、验证变更和沉淀规则。
...
```bash
pipx install infharness
```

也可以使用 `uv`：

```bash
uv tool install infharness
```

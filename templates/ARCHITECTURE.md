...
- 完成后同步检查 [AGENTS.md](AGENTS.md)、[RULES.md](RULES.md)、本地 skills 和验证入口中引用的路径是否一致。
...
{% elif HAS_CLAUDE_INTEGRATION %}
- [`CLAUDE.md`](CLAUDE.md)：Claude Code companion 指南，默认应指向 [AGENTS.md](AGENTS.md)。
- [NEEDS CLARIFICATION: 验证入口尚未由 HarnessKit 安装；请记录当前仓库已有的 test、lint、build 或 review 入口]
{% else %}
- [NEEDS CLARIFICATION: agent integration 尚未安装；如后续接入 Codex 可通过 `harnesskit integration install codex` 添加本地 skills 和验证入口，接入 Claude Code 可通过 `harnesskit integration install claude` 添加 companion 指南]
- [NEEDS CLARIFICATION: 验证入口尚未由 HarnessKit 安装；请记录当前仓库已有的 test、lint、build 或 review 入口]
{% endif %}

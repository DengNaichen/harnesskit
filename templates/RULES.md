...
`.harnesskit/facts.md` 是 `$scan-facts` 生成的事实交接快照，可作为补全本文件的 evidence input；它不是仓库事实本身的替代品。启用任何 Rule 或 Guard 前，仍需核对真实仓库文件、脚本、CI/hook 配置或团队确认来源。
...
- **facts 输入**：可以引用 `.harnesskit/facts.md` 中的扫描发现，但关键规则仍要写明真实仓库证据。

...
只有当任务多步骤、跨多个文件、涉及新功能或重构，且会影响最新发布标签中的行为或已发布/明确支持的持久外部状态边界时，才使用 ExecPlan。

如果仓库包含 `PLANS.md`，计划必须从其中的模板和规则开始，并在执行过程中维护进度、发现、决策和结果。如果仓库没有 `PLANS.md`，不要临时发明格式；先向用户说明缺失，再采用更轻量的计划说明。
